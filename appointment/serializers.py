...
class AppointmentListSerializer(serializers.ModelSerializer):
    patient_name = serializers.CharField(
        source="patient.base_user.full_name", read_only=True
    )
    doctor_name = serializers.CharField(
        source="doctor.base_user.full_name", read_only=True
    )
    doctor_specialty = serializers.CharField(
        source="doctor.specialty", read_only=True, allow_null=True
    )
    medical_service_name = serializers.CharField(
        source="medical_service.title", read_only=True, allow_null=True
    )
    section_name = serializers.CharField(
        source="section.title", read_only=True, allow_null=True
    )
...
class AppointmentCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if appointment_date < timezone.now().date():
            raise serializers.ValidationError(
                {"appointment_date": "تاریخ انتخابی نمی‌تواند در گذشته باشد"}
            )

        if (
            appointment_date == timezone.now().date()
            and appointment_time < timezone.now().time()
        ):
            raise serializers.ValidationError(
                {"appointment_start_time": "زمان انتخابی نمی‌تواند در گذشته باشد"}
            )
        ...
        if overlapping.exists():
            raise serializers.ValidationError(
                {"appointment_start_time": "این زمان قبلاً رزرو شده است"}
            )

        weekday = appointment_date.weekday()
        weekly_schedule = WeeklySchedule.objects.filter(
            doctor=data["doctor"], weekday=weekday, active=True
        ).first()
        ...
        if (
            appointment_time < weekly_schedule.start_time
            or appointment_time > weekly_schedule.end_time
        ):
            raise serializers.ValidationError(
                {
        ...
        exception = ExceptionDate.objects.filter(
            doctor=data["doctor"], date=appointment_date
        ).first()

        if exception and not exception.is_available:
            raise serializers.ValidationError(
                {"doctor": "پزشک در این تاریخ مرخصی دارد"}
            )
    ...
    def create_appointment_blocks(self, appointment):
        if appointment.appointment_start_time and appointment.total_duration:
            ...
            interval = timedelta(
                minutes=appointment.total_duration // 4
            )  # تقسیم به 4 بخش
    ...
    def validate(self, data):
        if "status" in data and data["status"] == AppointmentStatus.cancelled:
            if not data.get("cancellation_reason"):
                raise serializers.ValidationError(
                    {"cancellation_reason": "لطفاً دلیل لغو نوبت را وارد کنید"}
                )
        return data
...
class PatientOwnAppointmentSerializer(BaseUserSerializer):
    ...
    medical_service = serializers.CharField(
        source="medical_service.title", read_only=True
    )
