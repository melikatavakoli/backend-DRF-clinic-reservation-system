    ...
    def save(self, *args, **kwargs):
        if not self.title:
            ...
            with transaction.atomic():
                last_appointment = Appointment.objects.select_for_update().filter(
                    title__startswith=f"APT{today}"
    ...
    def clean(self):
        if self.doctor and self.appointment_date and self.appointment_start_time:
            ...
            if overlapping.exists():
                raise ValidationError(_("این زمان برای پزشک انتخاب شده قبلاً رزرو شده است"))
